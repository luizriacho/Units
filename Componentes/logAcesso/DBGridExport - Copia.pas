...
type
  ...
  private
    ...
    // Botão de Minimizar / Expandir Grid
    FBtnToggleGrid: TSpeedButton;

    // Configurações de Visibilidade
    FExibirExportarExcel: Boolean;
    FExibirImprimir: Boolean;
    FExibirDataInicio: Boolean;
    FExibirDataFim: Boolean;
    FExibirPesquisa: Boolean;
    FExibirBtnConsultar: Boolean;
    FExibirBtnToggleGrid: Boolean;
    ...
    procedure SetExibirDataInicio(const Value: Boolean);
    procedure SetExibirDataFim(const Value: Boolean);
    procedure SetExibirPesquisa(const Value: Boolean);
    procedure SetExibirBtnConsultar(const Value: Boolean);
    procedure SetExibirBtnToggleGrid(const Value: Boolean);
    ...
    function GetTextoPesquisa: string;
    procedure SetTextoPesquisa(const Value: string);
  ...
  published
    ...
    // Visibilidade dos botões padrão
    ...
    // Visibilidade dos componentes de Filtro
    property ExibirDataInicio: Boolean read FExibirDataInicio write SetExibirDataInicio default False;
    property ExibirDataFim: Boolean read FExibirDataFim write SetExibirDataFim default False;
    property ExibirPesquisa: Boolean read FExibirPesquisa write SetExibirPesquisa default False;
    property ExibirBtnConsultar: Boolean read FExibirBtnConsultar write SetExibirBtnConsultar default False;

    // Visibilidade do Botão de Minimizar / Expandir Grid
    property ExibirBtnToggleGrid: Boolean read FExibirBtnToggleGrid write SetExibirBtnToggleGrid default True;
    ...
    property TextoPesquisa: string read GetTextoPesquisa write SetTextoPesquisa;
...
begin
  ...
  FExibirDataInicio := False;
  FExibirDataFim := False;
  FExibirPesquisa := False;
  FExibirBtnConsultar := False;
  FExibirBtnToggleGrid := True;
...
begin
  inherited Resize;
  if Assigned(FBtnToggleGrid) and Assigned(FPanelBotoes) then
  begin
    FBtnToggleGrid.Left := FPanelBotoes.Width - FBtnToggleGrid.Width - 6;
  end;
end;
...
procedure TDBGridExport.SetExibirDataInicio(const Value: Boolean);
begin
  if FExibirDataInicio <> Value then
  begin
    FExibirDataInicio := Value;
    AjustarVisibilidadeBotoes;
  end;
end;

procedure TDBGridExport.SetExibirDataFim(const Value: Boolean);
begin
  if FExibirDataFim <> Value then
  begin
    FExibirDataFim := Value;
    AjustarVisibilidadeBotoes;
  end;
end;

procedure TDBGridExport.SetExibirPesquisa(const Value: Boolean);
begin
  if FExibirPesquisa <> Value then
  begin
    FExibirPesquisa := Value;
    AjustarVisibilidadeBotoes;
  end;
end;

procedure TDBGridExport.SetExibirBtnConsultar(const Value: Boolean);
begin
  if FExibirBtnConsultar <> Value then
  begin
    FExibirBtnConsultar := Value;
    AjustarVisibilidadeBotoes;
  end;
end;

procedure TDBGridExport.SetExibirBtnToggleGrid(const Value: Boolean);
begin
  if FExibirBtnToggleGrid <> Value then
  begin
    FExibirBtnToggleGrid := Value;
    AjustarVisibilidadeBotoes;
  end;
end;

function TDBGridExport.GetDataInicio: TDateTime;
begin
  Result := FDateEditInicio.Date;
end;

procedure TDBGridExport.SetDataInicio(const Value: TDateTime);
begin
  FDateEditInicio.Date := Value;
end;

function TDBGridExport.GetDataFim: TDateTime;
begin
  Result := FDateEditFim.Date;
end;

procedure TDBGridExport.SetDataFim(const Value: TDateTime);
begin
  FDateEditFim.Date := Value;
end;

function TDBGridExport.GetTextoPesquisa: string;
begin
  Result := FEditPesquisa.Text;
end;

procedure TDBGridExport.SetTextoPesquisa(const Value: string);
begin
...
begin
  // Panel Superior para Botões e Filtros
  ...
  FEditPesquisa.Parent := FPanelBotoes;
  FEditPesquisa.Top := 6;
  FEditPesquisa.Width := 150;
  FEditPesquisa.Height := 24;

  // Botão Consultar
  FBtnConsultar := TBitBtn.Create(Self);
  FBtnConsultar.Parent := FPanelBotoes;
  FBtnConsultar.Top := 6;
  FBtnConsultar.Width := 90;
  FBtnConsultar.Height := 26;
  ...
  // Botão Toggle (Minimizar / Expandir Grid)
  FBtnToggleGrid := TSpeedButton.Create(Self);
  FBtnToggleGrid.Parent := FPanelBotoes;
  FBtnToggleGrid.Top := 6;
  FBtnToggleGrid.Width := 26;
  FBtnToggleGrid.Height := 26;
  FBtnToggleGrid.Anchors := [akTop, akRight];
  FBtnToggleGrid.Flat := True;
  FBtnToggleGrid.OnClick := OnBtnToggleGridClick;
...
begin
  if Assigned(FPanelGrid) and FPanelGrid.Visible then
  begin
    FBtnToggleGrid.Caption := '▲';
    FBtnToggleGrid.Hint := 'Recolher Grid';
  end
  else
  begin
    FBtnToggleGrid.Caption := '▼';
    FBtnToggleGrid.Hint := 'Expandir Grid';
  end;
  FBtnToggleGrid.ShowHint := True;
end;

procedure TDBGridExport.OnBtnToggleGridClick(Sender: TObject);
begin
  SetGridVisible(not FPanelGrid.Visible);
end;
...
begin
  ...
  // Data Início
  FDateEditInicio.Visible := FExibirDataInicio;
  if FExibirDataInicio then
  begin
    FDateEditInicio.Left := vPosEsquerda;
    vPosEsquerda := vPosEsquerda + FDateEditInicio.Width + 6;
  end;

  // Data Fim
  FDateEditFim.Visible := FExibirDataFim;
  if FExibirDataFim then
  begin
    FDateEditFim.Left := vPosEsquerda;
    vPosEsquerda := vPosEsquerda + FDateEditFim.Width + 6;
  end;

  // Texto Pesquisa
  FEditPesquisa.Visible := FExibirPesquisa;
  if FExibirPesquisa then
  begin
    FEditPesquisa.Left := vPosEsquerda;
    vPosEsquerda := vPosEsquerda + FEditPesquisa.Width + 6;
  end;

  // Botão Consultar
  FBtnConsultar.Visible := FExibirBtnConsultar;
  if FExibirBtnConsultar then
  begin
    FBtnConsultar.Left := vPosEsquerda;
    vPosEsquerda := vPosEsquerda + FBtnConsultar.Width + 6;
  end;
  ...
  // Botão Toggle (Alinhado no canto direito)
  FBtnToggleGrid.Visible := FExibirBtnToggleGrid;
  FBtnToggleGrid.Left := FPanelBotoes.Width - FBtnToggleGrid.Width - 6;

  FPanelBotoes.Visible := FExibirDataInicio or FExibirDataFim or FExibirPesquisa or
                          FExibirBtnConsultar or FExibirExportarExcel or FExibirImprimir or FExibirBtnToggleGrid;
end;

procedure TDBGridExport.OnBtnConsultarClick(Sender: TObject);
begin
  if Assigned(FOnConsultar) then
    FOnConsultar(Self);
end;
